...
If the compiler finds a problem, it prints a DeepApp error code. For example, querying a `@no_index` field can produce a `DB021` error, using an unsafe `IN` query shape can produce a `DB022` error, and notifying an undeclared channel can produce `NOTIFY002`.
